...
aa_vec <- terra :: vect(aa, geom = c("Latitude", "Longitude"), crs = "EPSG:4326") #making a spat_raster
plot(aa_vec)
...
```{r}
#splitting into training data --> aa
train_indices_aa <- sample(1:nrow(aa), size = 0.8 *nrow(aa)) #taking a random sample from aa
train_data_aa <- aa[train_indices,]
test_data_aa <- aa[-train_indices,]
nrow(train_data_aa) 
nrow(test_data_aa) 

write.csv(train_data, file = here("Natalie/data_cleaned/test_train/train_data_aa.csv"))
write.csv(test_data, file = here("Natalie/data_cleaned/test_train/test_data_aa.csv"))



# Ct
train_indices_ct <- sample(1:nrow(ct), size = 0.8 *nrow(ct)) #taking a random sample from aa
...
#AA
train_indices_aa <- sample(1:nrow(aa), size = 0.8 *nrow(ct)) #taking a random sample from aa
train_data_ct <- ct[train_indices,]
test_data_ct <- ct[-train_indices,]
nrow(train_data_ct) 
nrow(test_data_ct) 
...
GLM - aa
```{r}
glm_model_aa2_h20 <- glm(aa ~ NDVI+ MNDWI+
                                 tavg2+ tmin2+ tmax2+
                                 prec2+ humd2, data = train_data, family = binomial(link = "logit"))

glm_model_aa1_h2o <- glm(aa ~ NDVI+ MNDWI+
                                 tavg1+ tmin1+ tmax1+
                                 prec1+ humd1, data = train_data, family = binomial(link = "logit"))

glm_model_aa2 <- glm(aa ~ tavg2+ tmin2+ tmax2+
                                 prec2+ humd2, data = train_data, family = binomial(link = "logit"))


glm_model_aa1 <- glm(aa ~ tavg1+ tmin1+ tmax1+
                                 prec1+ humd1, data = train_data, family = binomial(link = "logit"))
...
rf_model_aa1_h2o <- randomForest(x = train_data[, c("NDVI","MNDWI",
                       "tavg1","tmin1", "tmax1", "prec1", "humd1")], y=as.factor(train_data$aa), nodesize = 10, ntree = 100)

varImpPlot(rf_model_aa1_h2o)

rf_model_aa2_h2o <- randomForest(x = train_data[, c("NDVI","MNDWI",
                      "tavg2","tmin2", "tmax2", "prec2", "humd2")], y=as.factor(train_data$aa), nodesize = 10, ntree = 100)
varImpPlot(rf_model_aa2_h2o)

rf_model_aa1 <- randomForest(x = train_data[, c("tavg1","tmin1", "tmax1", "prec1", "humd1")], y=as.factor(train_data$aa), nodesize = 10, ntree = 100)
varImpPlot(rf_model_aa1)

rf_model_aa2 <- randomForest(x = train_data[, c("tavg2","tmin2", "tmax2", "prec2", "humd2")], y=as.factor(train_data$aa), nodesize = 10, ntree = 100)
varImpPlot(rf_model_aa2)
